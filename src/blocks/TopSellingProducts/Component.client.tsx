...
function TopSellingCard({ product }: { product: Partial<Product> }) {
  ...
  const buyNow = useCallback(() => {
    ...
    if (!product.id || isSoldOut) return
    void Promise.resolve(addItem({ product: product.id })).then(() => {
      router.push('/checkout')
    })
  }, [addItem, enableVariants, itemURL, isSoldOut, product.id, router])
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col justify-between gap-3 px-0.5 sm:gap-4 lg:pr-6 lg:pl-0">
          ...
          <div className="mt-1 grid w-full min-w-0 grid-cols-1 gap-2 lg:mt-0">
            ...
            <button
              ...
              disabled={enableVariants ? isLoading : !product.id || isSoldOut || isLoading}
              onClick={buyNow}
              type="button"
            >
