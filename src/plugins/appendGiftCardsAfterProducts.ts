...
export function appendGiftCardsAfterProductsPlugin(): Plugin {
  return (incomingConfig: Config): Config => {
    const existing = [...(incomingConfig.collections ?? [])]
    const without = existing.filter((c) => getCollectionSlug(c) !== 'gift-cards')
    ...
    return {
      ...incomingConfig,
      collections: [...without.slice(0, insertAt), GiftCards, ...without.slice(insertAt)],
    }
  }
}
