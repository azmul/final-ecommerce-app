...
export function ProductDeliveryEta({ estimatedDelivery }: Props) {
  const customEstimate = estimatedDelivery?.trim()
  const eta = estimateDeliveryEta({ area: 'dhaka', deliveryType: 'home' })
  const detail =
    customEstimate ||
    `${eta.label}. Exact timing depends on your district at checkout.`
