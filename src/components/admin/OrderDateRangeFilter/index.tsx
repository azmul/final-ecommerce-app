...
const startDateParam = 'orderStartDate'
const endDateParam = 'orderEndDate'
...
export const OrderDateRangeFilter = () => {
  ...
  const downloadCSV = async () => {
    ...
    try {
      // Match the list query: prefer applied URL filters over in-progress input edits
      ...
      while (hasNextPage) {
        ...
        if (!response.ok) {
          throw new Error('Unable to download orders.')
        }

        const result = await response.json()
        allOrders.push(...(Array.isArray(result.docs) ? result.docs : []))
      ...
      const rows = allOrders.map((order) => [
        ...
        order.status,
        getRelationLabel(order.customer as PrintableValue),
        ...
        getAddress(order.shippingAddress as PrintableValue),
      ])
      ...
      downloadFile(`orders-${rangeStart || 'all'}-to-${rangeEnd || 'all'}.csv`, csv)
    } catch (error) {
      console.error(error)
      window.alert('Unable to download orders CSV. Please try again.')
    } finally {
  ...
  return (
    ...
    >
      <button
        aria-controls="order-date-filter-panel"
      ...
      >
        ...
        <span style={{ flex: 1, minWidth: 0 }}>
          ...
          {isExpanded ? (
            ...
            >
              Select an order date range, then export the matching orders as a CSV file.
            </p>
          ) : null}
        </span>
      </button>
      ...
      {isExpanded ? (
        <div id="order-date-filter-panel">
          <div
            style={{
              alignItems: 'end',
              display: 'flex',
