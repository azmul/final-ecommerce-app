...
const startDateParam = 'productStartDate'
const endDateParam = 'productEndDate'
...
const formatCurrencyBDT = (amount: unknown): string => {
...
const getCategories = (categories: unknown): string => {
  if (!Array.isArray(categories)) {
    return ''
  }

  return categories
    .map((c) => getRelationLabel(c as PrintableValue))
    .filter(Boolean)
    .join('; ')
}
...
export const ProductDateRangeFilter = () => {
  ...
  const downloadCSV = async () => {
    ...
    try {
      ...
      while (hasNextPage) {
        ...
        if (!response.ok) {
          throw new Error('Unable to download products.')
        }

        const result = await response.json()
        allProducts.push(...(Array.isArray(result.docs) ? result.docs : []))
      ...
      const rows = allProducts.map((product) => [
        ...
        product.enableVariants === true ? 'Yes' : product.enableVariants === false ? 'No' : '',
        formatCurrencyBDT(product.priceInBDT),
        product.inventory,
      ...
      downloadFile(`products-${rangeStart || 'all'}-to-${rangeEnd || 'all'}.csv`, csv)
    } catch (error) {
      console.error(error)
      window.alert('Unable to download products CSV. Please try again.')
    } finally {
  ...
  return (
    ...
    >
      <button
        aria-controls="product-date-filter-panel"
      ...
      >
        ...
        <span style={{ flex: 1, minWidth: 0 }}>
          ...
          {isExpanded ? (
            ...
            >
              Select a creation date range, then export the matching products as a CSV file.
            </p>
          ) : null}
        </span>
      </button>
      ...
      {isExpanded ? (
        <div id="product-date-filter-panel">
          <div
            style={{
              alignItems: 'end',
              display: 'flex',
