...
const startDateParam = 'cartStartDate'
const endDateParam = 'cartEndDate'
...
export const CartDateRangeFilter = () => {
  ...
  const downloadCSV = async () => {
    ...
    try {
      ...
      const allCarts: Record<string, unknown>[] = []
      let page = 1
      ...
      while (hasNextPage) {
        ...
        if (!response.ok) {
          throw new Error('Unable to download carts.')
        }

        const result = await response.json()
        allCarts.push(...(Array.isArray(result.docs) ? result.docs : []))
      ...
      const rows = allCarts.map((cart) => [
        cart.id,
        formatDateTime(cart.createdAt),
        cart.status,
        getRelationLabel(cart.customer as PrintableValue),
        formatCurrency(cart.subtotal, cart.currency),
        cart.currency,
        getItems(cart.items),
        formatDateTime(cart.purchasedAt),
      ])
      ...
      downloadFile(`carts-${rangeStart || 'all'}-to-${rangeEnd || 'all'}.csv`, csv)
    } catch (error) {
      console.error(error)
      window.alert('Unable to download carts CSV. Please try again.')
    } finally {
  ...
  return (
    ...
    >
      <button
        aria-controls="cart-date-filter-panel"
      ...
      >
        ...
        <span style={{ flex: 1, minWidth: 0 }}>
          ...
          {isExpanded ? (
            ...
            >
              Select a cart creation date range, then export the matching carts as a CSV file.
            </p>
          ) : null}
        </span>
      </button>
