...
const startDateParam = 'wishlistStartDate'
const endDateParam = 'wishlistEndDate'
...
const formatProducts = (products: unknown): string => {
  if (!Array.isArray(products)) {
    return ''
  }

  return products
    .map((p) => getRelationLabel(p as PrintableValue))
    .filter(Boolean)
    .join('; ')
}
...
export const WishlistDateRangeFilter = () => {
  ...
  const downloadCSV = async () => {
    ...
    try {
      ...
      const allWishlists: Record<string, unknown>[] = []
      let page = 1
      ...
      while (hasNextPage) {
        ...
        const response = await fetch(`/api/wishlists?${params.toString()}`, {
          credentials: 'same-origin',
        })

        if (!response.ok) {
          throw new Error('Unable to download wishlists.')
        }

        const result = await response.json()
        allWishlists.push(...(Array.isArray(result.docs) ? result.docs : []))
        hasNextPage = Boolean(result.hasNextPage)
        page += 1
      }

      const headers = ['Wishlist ID', 'Customer', 'Products', 'Created', 'Updated']

      const rows = allWishlists.map((doc) => [
        doc.id,
        getRelationLabel(doc.customer as PrintableValue),
        formatProducts(doc.products),
      ...
      downloadFile(`wishlists-${rangeStart || 'all'}-to-${rangeEnd || 'all'}.csv`, csv)
    } catch (error) {
      console.error(error)
      window.alert('Unable to download wishlists CSV. Please try again.')
    } finally {
  ...
  return (
    ...
    >
      <button
        aria-controls="wishlist-date-filter-panel"
      ...
      >
        ...
        <span style={{ flex: 1, minWidth: 0 }}>
          ...
          {isExpanded ? (
            ...
            >
              Select a creation date range, then export the matching wishlists as a CSV file.
            </p>
          ) : null}
        </span>
      </button>
      ...
      {isExpanded ? (
        <div id="wishlist-date-filter-panel">
          <div
            style={{
              alignItems: 'end',
              display: 'flex',
