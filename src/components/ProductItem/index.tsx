...
export const ProductItem: React.FC<Props> = ({
  ...
  const itemPrice = variant?.priceInBDT || product.priceInBDT
  const itemURL = `/products/${product.slug}${variant ? `?variant=${variant.id}` : ''}`

  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
      <div className="flex h-20 w-20 shrink-0 items-stretch justify-stretch p-2 rounded-lg border">
      ...
      <div className="flex min-w-0 grow flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <div className="min-w-0 flex flex-col gap-1">
          <p className="font-medium text-base sm:text-lg wrap-anywhere">
            <Link href={itemURL}>{title}</Link>
          </p>
          {variant && (
            <p className="text-sm font-mono text-primary/50 tracking-widest wrap-anywhere">
              {variant.options
                ?.map((option) => {
