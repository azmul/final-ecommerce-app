...
export const AccountForm: React.FC = () => {
      ...
      if (user) {
        ...
        if (response.ok) {
          ...
          const phoneFromEmail =
            decodedLoginContact !== json.doc.email ? decodedLoginContact : undefined
          reset({
            name: json.doc.name,
            phone: json.doc.phone || phoneFromEmail || '',
            email: phoneFromEmail ? '' : json.doc.email,
            password: '',
            passwordConfirm: '',
          })
        } else {
  ...
  useEffect(() => {
    ...
    if (user) {
      const decodedLoginContact = loginEmailToContact(user.email)
      const phoneFromEmail = decodedLoginContact !== user.email ? decodedLoginContact : undefined
      reset({
        name: user.name,
        phone: user.phone || phoneFromEmail || '',
        email: phoneFromEmail ? '' : user.email,
        password: '',
        passwordConfirm: '',
      })
    }
