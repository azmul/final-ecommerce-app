...
function supportsServerFastRefreshConfig(version) {
  if (!version) {
    return false;
  }
  const {
...
var withPayloadLegacy = (nextConfig = {}) => {
  ...
  if (isBuild && (isTurbopackNextjs15 || isTurbopackNextjs16)) {
    throw new Error("Payload: Your Next.js version does not support using Turbopack for production builds. The *minimum* Next.js version required for Turbopack Builds is 16.1.0. Please upgrade to the latest supported Next.js version to resolve this error.");
  }
  const toReturn = {
    ...nextConfig,
...
var withPayload = (nextConfig = {}, options = {}) => {
  ...
  if (nextjsVersion?.major === 16 && !hasServerFastRefreshConfigOption) {
    console.warn("Payload: You are using an unsupported Next.js 16 version. You can find the supported Next.js versions here: https://payloadcms.com/docs/getting-started/installation");
  }
  const consoleWarn = console.warn;
  ...
  const baseConfig = {
    ...
    experimental: {
      ...
      ...hasServerFastRefreshConfigOption ? {
